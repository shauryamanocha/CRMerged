...
//    double localRadius = 10;
    ...
    Stats statsPage;
    ArrayList<LatLng> heatMapArrayList = new ArrayList<>();
    HeatmapTileProvider heatmapTileProvider;
    TileOverlay tileOverlay;
    private ClusterManager<customLatLng> clusterManager;
...
//                mMap.addMarker(new MarkerOptions().position(options.getPosition()).title(options.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.star)));
            ...
            user.amount++;
            if (user != null) {
                db.collection("Schools").document((String) user.school.get("name")).get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                    @Override
                    public void onSuccess(DocumentSnapshot documentSnapshot) {
                        if (documentSnapshot.getData() == null || documentSnapshot.getData().get("name") == null || documentSnapshot.getData().get("amount") == null) {
                            user.school.put("amount", 1);
                            db.collection("Schools").document(user.schoolName).set(user.school);
                            Log.w("schools", "noAmount");
                            Log.w("schools", (String) user.school.get("name"));

                        } else if (documentSnapshot.getData().get("amount") != null) {
                            user.school = (HashMap<String, Object>) documentSnapshot.getData();
                            user.school.put("amount", (int) (long) user.school.get("amount") + 1);
                            db.collection("Schools").document(user.schoolName).set(user.school);
                            Log.w("schools", "increaseAmount");
                        }
                        Log.w("tag", "read");
                    }
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Log.w("schools", "failed");
                    }
                });
            } else {
                Log.w("broken", "user is null");
            }
            user.hits.add(latlng);
            updateDatabase();
            heatMapArrayList.add(latlng.toLatLng());
            updateHeatMap();
            clusterManager.addItem(latlng);
...
//            mMap.addMarker(new MarkerOptions().position(options.getPosition()).title(options.getTitle()).icon(BitmapDescriptorFactory.fromResource(R.drawable.star)));
    ...
    public void updateDatabase() {
        db.collection("Schools").document((String) user.school.get("name"))
...
//        db.collection("References").add(reference);
        db.collection("References").document(user.name + " " + user.id).set(new com.example.shauryamanocha.mapmarkers.HitReference(user.schoolName, "User " + user.id));
        updateGraph();
    }
                ...
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        for (QueryDocumentSnapshot doc : task.getResult()) {
                            Log.w("Stats", "Amount :" + doc.getData().get("amount") + " School:" + doc.getData().get("name"));
                            Log.w("TAG", "User School: " + user.school);
//                             Stats.populate((int)(long)doc.getData().get("amount") + 0);
//                             statsPage.populate((int)(long)doc.getData().get("amount") + 0);
                            statsPage.populate(new graphPoint((String) doc.getData().get("name"), (int) (long) doc.getData().get("amount")));
//
                        }
                    }
                }).addOnFailureListener(new OnFailureListener() {
        ...
        if (statsPage._graph != null) {
            statsPage.numPoints = 0;
            statsPage._graph.clear();
            statsPage.entries.clear();
            statsPage.xLabels.clear();
        }
    }
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            if (task.isSuccessful()) {
                                ...
                                currentReference.get().addOnSuccessListener(new OnSuccessListener<DocumentSnapshot>() {
                                    @Override
                                    public void onSuccess(DocumentSnapshot documentSnapshot) {
//                                        readDataBase(documentSnapshot);
//                                        Log.i("TAG",".get");
                                    }
                                });
                                currentReference.addSnapshotListener(new EventListener<DocumentSnapshot>() {
                                    @Override
                                    public void onEvent(@Nullable DocumentSnapshot documentSnapshot, @Nullable FirebaseFirestoreException e) {
                                        updateGraph();
                                        user.hits.clear();
                                        heatMapArrayList.clear();
                                        clusterManager.clearItems();
                                        if(statsPage.graph!=null) {
                                            statsPage.graph.clear();
                                        }
                                        readDataBase(documentSnapshot);
                                        Log.i("TAG","snapshot");
                                    }
    ...
    void updateHeatMap(){
        if(heatMapArrayList!=null && heatMapArrayList.size()!=0) {
            heatmapTileProvider.setData(heatMapArrayList);
        }
        tileOverlay.clearTileCache();
    }

    void readDataBase(DocumentSnapshot documentSnapshot){
        ...
        for (Map.Entry<String, Object> h : hitsHash.entrySet()) {
            ...
            user.hits.add(new customLatLng((double) currentHit.get("latitude"), (double) currentHit.get("longitude"), (long) currentHit.get("id"), (long) currentHit.get("amount"), (String) currentHit.get("user"), System.currentTimeMillis()));
            user.amount = user.hits.size();
            heatMapArrayList.add(new LatLng((double) currentHit.get("latitude"), (double) currentHit.get("longitude")));
            clusterManager.addItem(new customLatLng((double) currentHit.get("latitude"), (double) currentHit.get("longitude"), (long) currentHit.get("id"), (long) currentHit.get("amount"), (String) currentHit.get("user"), System.currentTimeMillis()));
            clusterManager.cluster();
        }


        int[] colors = {
                Color.argb(127,102, 225, 0), // green
